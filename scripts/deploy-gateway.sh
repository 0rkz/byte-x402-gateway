...
git diff --quiet HEAD -- src/ scripts/ 2>/dev/null || { echo "[deploy] ABORT: uncommitted MODIFIED source under src/ or scripts/ — commit or stash before deploying"; exit 1; }
...
UNTRACKED=$(git ls-files --others --exclude-standard -- src/ scripts/) || { echo "[deploy] ABORT: cannot enumerate untracked source (git failed)"; exit 1; }
