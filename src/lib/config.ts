...
export const feedRegistry: FeedMetadata[] = [
  ...
  // KYA repositioning 2026-08-12 (founder-authorized): the verdict cluster
  // (address-reputation / sanctions-screen / merchant-screen) leads with
  // Know-Your-Agent counterparty-screening framing. Each description carries a
  // scope clause — we screen the counterparty the caller supplies, we do NOT
  // verify the calling agent's identity. Copy change only; prices untouched.
  customPricedFeed("address-reputation", "Address Reputation Oracle", "Know-Your-Agent (KYA) counterparty screening — reputation pillar. Agentic-payments go/no-go verdict: synchronous signed ALLOW/WARN/BLOCK for (domain, receiving address, amount, chain) BEFORE releasing USDC. ar-v1 ruleset over RDAP/TLS/DNS/Wayback domain signals + on-chain receiving-address signals + curated known-bad blocklist. The verdict carries an embedded EIP-712 PayloadAttestation — recompute keccak256(answer) and recover the signer before acting. Scope: screens the counterparty tuple you supply — not identity verification of the calling agent.", "on-demand", 2500, "commerce", "100000"),
